...
func (svc *Service) GetSharedURL(objectName string, fileName string) (string, error) {
	// Set request parameters for content-disposition.
	reqParams := make(url.Values)
	reqParams.Set("response-content-disposition", fmt.Sprintf("attachment; filename=\"%s\"", fileName))

	// Generates a presigned url which expires in 3 minutes.
	presignedURL, err := svc.client.PresignedGetObject(context.Background(), svc.BucketName, objectName, time.Second*3*60, reqParams)
	if err != nil {
		return "", err
	}
